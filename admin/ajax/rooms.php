...
if(isset($_POST['add_room']))
{
   ...
   $room_id = mysqli_insert_id($con);
    ...
    if($stmt = mysqli_prepare($con, $q2)){
        ...
        $flag = 0;
        die('No se puede preparar la consulta - insert');
...
if(isset($_POST['get_all_rooms']))
{
    ...
    while($row = mysqli_fetch_assoc($res)){
        if($row['status']==1){
            $status = "<button onclick='toggle_status($row[id],0)' class='btn btn-dark btn-sm shadow-none' >Ocupado</button>
            ";
        }
        else{
            $status = "<button onclick='toggle_status($row[id],1)' class='btn btn-warning btn-sm shadow-none'>Libre</button>
            ";
        }
        $data.= "
